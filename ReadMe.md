...
- **Exit protection** — when changes are unsaved, closing, reloading, or navigating away requests the browser's standard confirmation dialog. The browser controls the final wording and lets the user stay or leave.
...
- **Search hierarchy and highlighting** — matching a parent shows its direct child tasks, while matching a child shows only its parent context. Matching title text is highlighted in bold bright red, including partial-word matches.
- **Search context guidance** — when no tasks match, the grid identifies the active left-panel view in bold red so users can recognize when another context may contain the task.
- **Active search indicator** — the Search field has a red border while it contains text, making an active text filter immediately apparent.
- **Task editor Notes placement** — Notes, including Expand Notes and Collapse Notes, appears directly below Task Title; all other editor fields retain their existing layout.
...
- **Recently Accessed** — opening an existing task records its last accessed time and marks the dataset unsaved. The left-panel view shows tasks opened during the rolling previous 24 hours, most recently accessed first. Older datasets without access history load normally and appear after their tasks are opened.
...
- **AI Efficiency** — the statistics section displays AI efficiency for qualifying terminal items (Completed or Closed), including archived tasks: total estimated time saved divided by total estimated effort without AI. Hover it for the overall Productivity Multiplier. The AI Analysis view and exported HTML include overall metrics plus a per-task Productivity Multiplier column. Terminal describes workflow state, not deletion.
